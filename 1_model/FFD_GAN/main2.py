...
if __name__ == '__main__':
    ...
    G = model.generator6(noise_dim).to(device)
    ...
    for epoch in range(train_epoch):
        ...
        for x_ in train_loader:
            ...
            for j in range(5):
                ...
                # R3       = -1*nn.MSELoss()(defort,target)
                # R3.backward(retain_graph=True)

                defort     = fake_img.data.float()
                target     = base_mesh.repeat(x_.shape[0],1,1,1).float()
                R3 =  nn.MSELoss()(fake_img,target).float()                 # R3.backward(retain_graph=True)
